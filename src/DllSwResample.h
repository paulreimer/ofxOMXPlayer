...
public:
  ...
  virtual int swr_convert(struct SwrContext *s, uint8_t **out, int out_count, uint8_t **in , int in_count){ return ::swr_convert(s, out, out_count, in, in_count); }
};
#else
...
public:
  ...
  virtual int swr_convert(struct SwrContext *s, uint8_t **out, int out_count, uint8_t **in , int in_count){ return ::avresample_convert(s, out, 0, out_count, in, 0,in_count); }
};
#endif
...
class DllSwResample : public DllDynamic, DllSwResampleInterface
{
  ...
  DEFINE_METHOD5(int, swr_convert, (struct SwrContext *p1, uint8_t **p2, int p3, uint8_t **p4, int p5))
